 ...
 * =====================================================
 * This is the first of three forks of ./mlst_network.h that introduce energy awareness (nodes with low energy are more likely to
 * become leaves).
 * 
 * This method chooses always the parent with the best energy state and only at second place for children count.
		...
		if(n_pv->parent_id == 0){//Neighbor has undefined state;
			...
			if( n_pv->parent_id == (RIME_ID) || n_pv->energy_state == 0 ){ //is defined child
				...
				if(n_pv->distance_to_root+1 < distance_to_root){//closer than current best parent
					...
					best_parent = n;
					best_parent_pv = n_pv;
				} else if(n_pv->distance_to_root+1 == distance_to_root){ //compare energy
