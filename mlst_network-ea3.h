/**
 * Energy Aware Maximum Leaf Spanning Tree, Algorithm 1
 * =====================================================
 * This is the third of three forks of ./mlst_network.h that introduce energy awareness (nodes with low energy are more likely to
 * become leaves).
 * 
 * This method weights edges based on the energy state and builds a routing tree (shortest path).
 * Edges to parents with low energy state a expensive.
...
static void onPvnDelete(struct Nbr* n)
{
	...
	if(n == mlst_parent){ //If parent is deleted, reset state
		...
		own_mlst_public_variable.distance_to_root = 0xffff;
...
static void mlst_recalculate(){
	...
	uint16_t distance_to_root = 0xffff;
	...
	for(; n!=0; n=pvn_getNextNbr(n)){
		...
		if(n_pv->parent_id == 0){//Neighbor has undefined state;
			...
			if( n_pv->parent_id == (RIME_ID) || n_pv->energy_state == 0 ){ //is defined child
				...
				if(n_pv->distance_to_root+n_pv->energy_state < distance_to_root){//closer than current best parent
					distance_to_root = n_pv->distance_to_root+1;
					number_of_potential_parents = 1;
					best_parent = n;
					best_parent_pv = n_pv;
				} else if(n_pv->distance_to_root+n_pv->energy_state == distance_to_root){ //compare energy
					if(best_parent_pv->energy_state > n_pv->energy_state){
						number_of_potential_parents = 1;
						best_parent = n;
						best_parent_pv = n_pv;
					} else if(best_parent_pv->energy_state > n_pv->energy_state){ 
